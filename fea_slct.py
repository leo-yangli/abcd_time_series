...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, dataset_root, parts, prediction_target, task):
        ...
        raw_labels = []
        for part in parts:
            raw_labels.append(pd.read_csv(f'/home/yli93/dataset/intersection_part{part}_new.csv'))
...
def train_one(epoch):
    ...
    for i, (img,  label, age) in enumerate(tbar):
        ...
        (loss + reg_loss * regcoef).backward()
        optimizer.step()
        loss_list.append(loss.item() / train_dataset.var)
    logging.info(f'Train {epoch}: mse {np.mean(loss_list)} reg {reg_loss.item()}')  
...
def val_one(epoch):
    ...
    with torch.no_grad():
        ...
        torch.save(model.state_dict(), f'{folder}/model')


        pd.DataFrame(train_samp.cpu().numpy()).to_csv(f'{folder}/{epoch}_{mse:.2f}_{corr:.2f}', header=False, index=False,)
    return np.mean(loss_list)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='ABCD LSTM Feature Selection')
    ...
    folder = f'logs/fea_selc_{task}/{test_fold}_{target}_{regcoef}'
    ...
    for epoch in range(args.epochs):
        avg_train_loss = train_one(epoch)
        avg_val_loss = val_one(epoch)
        scheduler.step()

    logging.info(f'train {train_folds}, test{test_fold}, task {task}, target {target}  finished...')
